---
title: 算法篇
date: 2020-04-09 09:50:06
permalink: /pages/9a4ae7/
categories:
  - 面经整理
  - 知识点整理
tags:
...
《JavaScript 高级程序设计》当中对 Array 类型的讲解：
[Array 类型详解](https://juejin.im/post/5e5a1e3cf265da574e229b25#heading-3)
...
let res = function() {
	...
	return arr2.filter((item) => obj[item] && obj[item] === item);;
};
```
...
对于 `nums` 中的每个数字，我们可以将其添加到 k 个子集中的一个，只要该子集的和不会超过目标值。对于每一个选择，我们都递归地用一个更小的数字进行搜索，以便在`nums`中考虑。如果我们成功地放置了每个数字，那么我们的搜索就成功了。
...
另一个细节是我们可以对数组 `num`进行排序，以便我们尝试先放置较大的元素。这种放置元素的方法将更快组合出较小大小的子集。我们还可以适当处理 `nums[i] >= target` 的情况。
...
执行一遍遍历，当大于等于 target 的时候即可输出；遍历完之后不符合条件，则在数组的结尾处插入！
...
> 给定一个字符串，返回其中两个小括号之间的最长子字符串

**思路**

通过栈的思想做匹配，注意这里只需要左边的括号入栈，遇到左边括号的时候存入索引，遇到右边括号时如果栈不为空，则弹出索引，返回 end-start 之间的字符串。
...
function check(str) {
	...
	for (const index of tem) {
		...
		if (str[i] == '(') {
			...
			max = end - p;
			res = res > max ? res : max;
			resStr = tem.slice(p, p + res + 1).join('');
		}
	}
	return resStr ? resStr : false;
}

console.log(check('dasdas(da(ds)sd)'));
```
...
方法1：我们使用两个指针 node1，node2 分别指向两个链表 headA，headB 的头结点，然后同时分别逐结点遍历，当 node1 到达链表 headA 的末尾时，重新定位到链表 headB 的头结点；当 node2 到达链表 headB 的末尾时，重新定位到链表 headA 的头结点。
...
// 方法2：
// 遍历所有链表元素，将元素值放到一个数组中; 排序数组，相当于按优先级排序优先级队列
...
[leetcode 第五题 ](https://leetcode-cn.com/problems/longest-palindromic-substring/submissions/)

这道题我一开始没做出来，在题解当中找到了我觉得能掌握的方法记录了下来。
...
> [ { id: 1, child: [
>
> ​			{ id: 2, child: [{ id: 4, child: [{ id: 5, child: [] }] }] },
...
```js
function listToTree(list, parent = 0) {
	var out = [];
...
先将传入的指定执行环境的对象 context 取到将需要执行的方法(调用call的对象) 作为 context 的一个属性方法fn
处理传入的参数， 并执行 context的属性方法fn， 传入处理好的参数删除私自定义的 fn 属性返回执行的结果。
...
节流：每隔一段时间执行一次（没有执行存在的情况下才新建执行，并在执行的时候清除存在的执行）

防抖：一段时间内只执行一次（清理掉正在执行的，重新执行）
...
浅拷贝在拷贝应用类型的值的时候拷贝的是应用类型的地址，在新的对象当中的改动操作的是相同的对内存空间，故相互影响。深拷贝创建不用的内存地址，拷贝之后的值修改不会造成相互影响。

```js
// 第一种：JSON.parse(JSON.stringify(Object))方法实现深拷贝

// 第二种：递归的方法实现深拷贝
function deepclone(obj, copyobj){
    var copyobj=copyobj||{};
  	// 使用for..in进行遍历 数组的话keys为0,1..... keys是string类型
    for (var keys in obj) { 
      	// 判断是否是自带属性，去除原型链上的属性
...
function mysetinterval(fn, time){
    ...
    function interval(){//执行该函数，异步被挂起time时间后在执行，一上来就执行fn
        setTimeout(interval,time);// 异步 循环执行
        fn;
    }
