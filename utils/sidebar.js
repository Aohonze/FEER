...
function main() {
	...
	const sidebarDataTem = ejs.render(sidebarTemplate, {
		sidebarData,
		catalogueData,
	});
...
function mapTocToSidebar(root, prefix) {
	...
	files.forEach((filename) => {
		...
		if (isNaN(order) || order < 0) {
			logger.error(
				`该文件 "${file}" 序号出错，请填写正确的序号，序号约定请查看：https://github.com/xugaoyi/vuepress-theme-vdoing/issues/113`
			);
			return;
		}
		if (sidebar[order]) {
